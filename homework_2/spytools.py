...
def gen_consecutive_chars(start=97, end=122):
    ...
    for character in range(start, end + 1):
        consecutive_characters += chr(character)
    return consecutive_characters
...
def gen_key(password):
    ...
    new_password = remove_duplicates(password)
    last_character = new_password[-1]
    last_character_index = alphabet.find(last_character)
    second_alphabet = alphabet[last_character_index +1:]
    first_alphabet = alphabet[:last_character_index]
    key = new_password + remove_letters(new_password, second_alphabet) + remove_letters(new_password, first_alphabet)
    return key
...
def vig_decrypt(key, message):
    ...
    for i in range(len(message)):
        ...
        try:
            other_index = (alphabet.index(ch) - offset) % alphabet_len
            new_message += alphabet[other_index]
        except ValueError:
            new_message += ch
    return new_message
...
if __name__ == '__main__':
    ...
    print(sub_decrypt("topsecret", "le tge svhpbkeges. cyee ti bape."))
    print(vig_decrypt("davinci", "whz rcooe pnu oailrf"))
