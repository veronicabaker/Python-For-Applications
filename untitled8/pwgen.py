...
def generate_password_file(source_fn, dest_fn, rules=lambda word:true):
    ...
    * writes out combinations as username password in file
    ...
    :rtype: None
    """
    words = filter_words_from_file(source_fn, rules)
    combos = combinations(words)
...
def load_password_file(pw_fn, delimiter=':'):
    ...
    """
    new_list = []
    with open(pw_fn, "r") as f:
        for line in f.readlines():
            line = line.strip()
            line = line.split(delimiter)
            temp_tuple = line[0], line[1]
            new_list.append(temp_tuple)
...
if __name__ == "__main__":
    ...
    * call generate_password_file with the rules function that you've
    ...
    """
    def password_rules(x):
        if x.count("s") >= 2 and "o" in x and (x[0] == "h" or x[0] == "v") and len(x) >= 14:
            return True
        return False
    generate_password_file("words.txt", "passwords.txt", password_rules)
